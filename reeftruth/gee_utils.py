...
def label_axis_bars_with_counts(ax, decimals=0):
    """
    Labels the bars in a bar plot with their respective counts.
    
    Args:
    ax (matplotlib.axes.Axes): The axes object containing the bar plot.
    decimals (int): The number of decimal places to display for the counts.
    """
    for rect in ax.patches:
        height = rect.get_height()
        ax.text(rect.get_x() + rect.get_width() / 2, height, f"{height:.{decimals}f}", ha="center", va="bottom")